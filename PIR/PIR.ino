...
void setup() {
  ...
  // Set up WebSocket for the first ESP32
  webSocket.begin("192.168.0.113", 81); // Replace with the IP of the first ESP32-CAM
  webSocket.onEvent(handleWebSocketEvent);
  webSocket.onEvent([](uint8_t num, WStype_t type, uint8_t *payload, size_t length) {
    handleWebSocketEvent(num, type, payload, length);
  });

  // Set up WebSocket for the second ESP32 (additionally)
  webSocket.begin("your-ESP32-CAM-IP-SECOND", 82); // Replace with the IP of the second ESP32-CAM
  webSocket.onEvent(handleWebSocketEventSecond);

  // Handlers for the first ESP32 WebSocket
  ...
  // Handlers for the second ESP32 WebSocket (additionally)
  server.on("/capture-second", HTTP_GET, [](AsyncWebServerRequest *request) {
    // Handle the second ESP32 capture logic
    // You can implement this based on your requirements
    request->send(200, "text/plain", "Capturing from the second ESP32");
  });
